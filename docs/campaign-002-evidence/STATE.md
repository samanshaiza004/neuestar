...
# Phase 3 — Campaign 002 FULL-VM PREFLIGHT (2026-08-16)

Specimen frozen: docs/CAMPAIGN-002.md (commit 40d1769, main). Identity: outer sha
a5773bc2…c10fe, payload b6f12734…, runtime-root 08487e1d…, capture-rule 6f71fc1b…,
probe 0.2.0, source 06adad5 (hosted build run 31979454518). PR #1 reviewed and
approved; merged to main as 06adad5; freeze record 40d1769.

Fresh external overlays (ubuntu-2604-c2.qcow2, nixos-2605-c2.qcow2) over the
pristine base disks; fresh NVRAM; fresh evidence directories per guest. Lab
credential rotated on the Ubuntu overlay (compromised password invalidated;
new credential at evidence/c2-lab-credential.txt, 0600) as ordinary lab
provisioning before the run; SSH-key-only access. Wayland session verified on
both overlays before the run (loginctl Type=wayland/Class=user/State=active,
live /run/user/1000/wayland-0 socket, compositor env via systemctl --user).
Specimen unchanged between guests. Ubuntu ran first.

| | ubuntu-2604-c2 (26.04, k7.0.0-29, userns-restrict=1) | nixos-2605-c2 (26.05, k6.18.44) |
|---|---|---|
| observed display | wayland (ubuntu:GNOME) | wayland (GNOME) |
| PROBE_EXIT | 71 | 0 |
| substage | helper-execution | (success) |
| process_stderr | `bwrap: setting up uid map: Permission denied` | — |
| L0.0 containment | fail | **pass** (user+mount ns ids differ; controlled root) |
| L0.1 launch | not-run | **pass** (controlled glibc 2.39, host_glibc_imported=false) |
| L0.2..L0.5 | not-run | not-run |
| classification | fail (containment) | fail (graphics not implemented — Phase 1 semantics) |

Interpretation: with the network-namespace over-scope removed, Ubuntu fails at
the minimum user-namespace boundary itself — stock AppArmor
...
NixOS, which in Campaign 001 only exposed the probe mount-plan defect, now
passes L0.0+L0.1 zero-preparation under the corrected contract — the first
clean minimum-containment milestone. Both results are FULL-VM PREFLIGHT
evidence, never physical Gate L0 evidence. Protocol followed: stop on L0.0
failure (Ubuntu) with no guest repair; guests left as-is; identical specimen
for both.
