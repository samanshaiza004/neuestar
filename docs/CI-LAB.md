...
runtime-root manifest identity, controlled glibc version/interpreter, payload
identity, outer archive checksum, and build time derived from the source
commit. The archive is uploaded once for a campaign. A failed physical cell
...
`display-*`. Churn runners also carry one unique `machine-*` label. Each
manually dispatched cell downloads the canonical archive,
...
Runner labels are operator assertions. Before a real attempt the launcher
rejects a declared distro that disagrees with `os-release` and a declared
display that disagrees with the observed session/display environment. GPU
identity remains unclaimed until Vulkan device enumeration exists in Phase 4;
Phase 1 therefore cannot classify a complete matrix cell as passing.
...
driver identity, and no Neuestar changes. The post-change dispatch must repeat
the baseline's machine label and app-specific machine-identity hash; both
phases reject UID 0. Captured concrete files may differ.
