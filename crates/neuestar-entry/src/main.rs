...
#[cfg(target_os = "linux")]
fn close_range_result() -> Option<i64> {
    // SAFETY: close_range(3, u32::MAX, 0) is a plain syscall on fd numbers;
    // it cannot affect memory or other processes.
    Some(unsafe { libc::syscall(libc::SYS_close_range, 3usize, u32::MAX as usize, 0usize) })
}
...
fn close_inherited_fds() {
    let rc = close_range_result();
    if rc == Some(0) {
        return;
    }
    let rc_description = rc.map_or_else(|| "unavailable".to_owned(), |value| value.to_string());
    let entries = match std::fs::read_dir("/proc/self/fd") {
        Ok(entries) => entries,
        Err(error) => {
            eprintln!("entry: cannot close inherited fds (close_range rc={rc_description}, /proc/self/fd: {error}); aborting");
            exit(2);
        }
    };
